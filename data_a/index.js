// V1-DATA-A-1.2 — OKX签名 + 429指数退避 + jitter + KV ready标记
...
async function run(){
  // A: 启动随机jitter 0-45秒，避免多Worker同时冲击OKX
  ...
  log('DATA_A_START','请求币数量: '+COINS.length);
  const kd={};let fail=[];
  // 先写busy标记
  await kvW('data_a',{status:'busy',ts:Date.now()});
  ...
  log('DATA_A_END','success='+ok+'/'+COINS.length+' failed='+fail.length+' total_time='+((Date.now()-st)/1000).toFixed(1)+'秒');
  // KV写入: 带ready标记
  log('KV_WRITE_START','key=data_a');
  const kvData={status:'ready',ts:Date.now(),d:kd};
  await kvW('data_a',kvData);
