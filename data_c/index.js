// V1-DATA-C-1.2 — OKX签名 + 429指数退避 + jitter + KV ready标记
const COINS=['HYPE','DOT','APT','ARB','OP','ATOM','NEIRO','GALA','PEPE','WIF'];
...
async function run(){
  ...
  log('DATA_C_START','请求币数量: '+COINS.length);
  const kd={};let fail=[];
  await kvW('data_c',{status:'busy',ts:Date.now()});
  ...
  log('KV_WRITE_START','key=data_c');
  const kvData={status:'ready',ts:Date.now(),d:kd};
  await kvW('data_c',kvData);
